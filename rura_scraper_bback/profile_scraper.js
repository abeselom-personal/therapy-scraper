...
const allproviders = JSON.parse(fs.readFileSync('./collected.json', 'utf-8'));
const providers = allproviders.flat();
...
const CONFIG = {
    ...
    headless: false,
    workers: Math.min(os.cpus().length - 1, 6),
    batchSize: 10
};
...
async function scrapeProviderData(provider, browserContext, srNo) {
    ...
    try {
        ...
        await page.route('**/*', (route) => {
            ...
            // Only block images, fonts, and media. Allow CSS and scripts.
